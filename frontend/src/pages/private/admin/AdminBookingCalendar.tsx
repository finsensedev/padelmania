...
export default function AdminBookingCalendar() {
  ...
  // Helper to extract equipment quantities from a booking
  const getEquipment = (booking: any) => {
    const rackets =
      booking.rackets?.quantity ||
    ...
    const packs =
      booking.equipmentRentals
        ...
        .reduce((sum: number, r: any) => sum + (r.quantity || 0), 0) || 0;

    return { rackets, packs };
  };

  // Fetch all bookings within the visible month for aggregation
  const monthStart = startOfWeek(startOfMonth(monthCursor), {
    weekStartsOn: 1,
  });
  const monthEnd = endOfWeek(endOfMonth(monthCursor), { weekStartsOn: 1 });
  ...
  const { data: monthBookings = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const bookings = await bookingService.list({
        ...
        limit: 1000, // Set high limit for calendar view to get all bookings
      });
      return bookings;
    },
  });

  // Group bookings by date string for calendar dots / counts
  interface DayCounts {
    total: number;
    cancelled: number;
    refunded: number;
  ...
  const bookingsByDate = useMemo(() => {
    ...
    for (const b of monthBookings) {
      const d = format(new Date(b.startTime), "yyyy-MM-dd");
      if (!map[d]) map[d] = { total: 0, cancelled: 0, refunded: 0, active: 0 };
      map[d].total += 1;
      if (b.status === "CANCELLED") map[d].cancelled += 1;
      else if (b.status === "REFUNDED") map[d].refunded += 1;
      else map[d].active += 1;
    }
    return map;
  }, [monthBookings]);

  // Bookings for selected date for detailed view
  ...
  const byHour = useMemo(() => {
    const map: Record<string, typeof dayBookings> = {};
    for (const b of dayBookings) {
      const h = format(new Date(b.startTime), "HH:00");
      map[h] = map[h] || [];
      map[h].push(b);
    }
    ...
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 bg-background h-full overflow-auto">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:gap-6">
        <Card className="flex-1 shadow-sm hover:shadow-md transition-shadow">
          <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0 p-4 sm:p-6 border-b border-border bg-muted/30">
            <div className="space-y-1">
              <CardTitle className="text-xl sm:text-2xl font-semibold">
                Bookings Calendar
              </CardTitle>
              <p className="text-xs sm:text-sm text-muted-foreground font-medium">
                Monthly overview with per-day counts
              </p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className="hover:bg-muted hover:border-primary/20 transition-colors"
                >
                  <ChevronLeft className="h-4 w-4" />
                </Button>
                <div className="min-w-[120px] sm:min-w-[140px] text-center font-semibold text-sm sm:text-base">
          ...
          <CardContent className="p-3 sm:p-6">
            <div className="grid grid-cols-7 text-xs font-semibold text-muted-foreground mb-2 bg-muted/50 rounded-t-lg">
                ...
                {weeks.map((week, wi) => (
                  <div key={wi} className="grid grid-cols-7">
                    {week.map((d) => {
                      ...
                      const counts = bookingsByDate[key];
                      const count = counts?.total || 0;
                      const isCurrentMonth = isSameMonth(d, monthCursor);
                        ...
                        <Popover key={key}>
                          <PopoverTrigger asChild>
                            <button
                              ...
                              className={`relative h-16 sm:h-20 border border-border flex flex-col items-start p-1 sm:p-1.5 text-left hover:bg-muted/50 focus:outline-none focus:ring-2 focus:ring-primary/40 transition-colors ${
                                !isCurrentMonth
                            ...
                            >
                              ...
                              {count > 0 && (
                                <div className="mt-auto flex gap-0.5 flex-wrap">
                                  {/* Active (non-cancelled/non-refunded) */}
                                  {counts.active > 0 && (
                                    <span
                                      className={`inline-flex items-center justify-center rounded-sm bg-success text-white text-[9px] px-1 leading-none h-4 min-w-[1.1rem] font-semibold ${
                                        selected ? "ring-1 ring-success" : ""
                                      }`}
                                    >
                                      {counts.active}
                                    </span>
                                  ...
                                  {counts.cancelled > 0 && (
                                    <span
                                      className={`inline-flex items-center justify-center rounded-sm bg-warning text-white text-[9px] px-1 leading-none h-4 min-w-[1.1rem] font-semibold ${
                                        selected ? "ring-1 ring-warning" : ""
                                      }`}
                                    >
                                      {counts.cancelled}
                                    </span>
                                  ...
                                  {counts.refunded > 0 && (
                                    <span
                                      className={`inline-flex items-center justify-center rounded-sm bg-destructive text-white text-[9px] px-1 leading-none h-4 min-w-[1.1rem] font-semibold ${
                                        selected
                          ...
                          {count > 0 && (
                            ...
                            >
                              ...
                              <div className="max-h-72 overflow-y-auto p-3 sm:p-4 space-y-2 text-xs">
                                {monthBookings
                                  .filter((b) =>
                                    isSameDay(parseISO(b.startTime), d)
                                  )
                                  .slice(0, 8)
                                  .map((b) => {
                                    const equipment = getEquipment(b);
                                    return (
                                      <div
                                        key={b.id}
                                        className="border border-border rounded-md p-2 bg-muted/30 hover:bg-muted/50 transition-colors"
                                      >
                                        ...
                                        <div className="text-[10px] uppercase tracking-wide flex items-center gap-2 mt-1 flex-wrap">
                                          <span className="px-1.5 py-0.5 bg-muted rounded font-semibold">
                                            {b.status}
                                          </span>
                                          {equipment.rackets > 0 && (
                                            <span className="inline-flex items-center gap-1 text-success bg-info/10 border border-info/20 rounded px-1.5 py-0.5 font-semibold">
                                              <svg
                                          ...
                                          {equipment.packs > 0 && (
                                            <span className="inline-flex items-center gap-1 text-success bg-success/10 border border-success/20 rounded px-1.5 py-0.5 font-semibold">
                                              🎾 {equipment.packs}
                                            </span>
                                          )}
                                        </div>
        ...
        <Card className="w-full lg:w-[420px] mt-4 lg:mt-0 shadow-sm hover:shadow-md transition-shadow">
            ...
            {isLoading ? (
              ...
              <div className="space-y-3 sm:space-y-4">
                {byHour.map(([hour, list]) => (
                  ...
                  >
                    <div className="px-3 sm:px-4 py-2 border-b border-border bg-muted/50 font-semibold text-xs sm:text-sm tracking-wide flex items-center justify-between">
                      <span>{hour}</span>
                      <span className="text-[10px] sm:text-xs text-muted-foreground font-medium">
                        {list.length} booking{list.length > 1 ? "s" : ""}
                      </span>
                    </div>
                    <div className="p-2 sm:p-3 space-y-2 bg-muted/20">
                      {list.map((b) => {
                        const equipment = getEquipment(b);
                        return (
                          <div
                            key={b.id}
                            className="p-2 sm:p-2.5 border border-border rounded-md bg-background hover:bg-muted/30 transition-colors"
                          >
                            ...
                            <div className="text-[11px] sm:text-xs text-muted-foreground flex gap-2 flex-wrap items-center mt-1 font-medium">
                              <span>
                                {format(new Date(b.startTime), "HH:mm")} -{" "}
                                {format(new Date(b.endTime), "HH:mm")}
                              </span>
                              <span className="uppercase tracking-wide px-1.5 py-0.5 bg-muted rounded font-semibold">
                                {b.status}
                              </span>
                              {equipment.rackets > 0 && (
                                <span className="inline-flex items-center gap-1 text-success bg-info/10 border border-info/20 rounded px-1.5 py-0.5 font-semibold">
                                  ...
                                  {equipment.rackets} racket
                                  {equipment.rackets === 1 ? "" : "s"}
                                </span>
                              )}
                              {equipment.packs > 0 && (
                                <span className="inline-flex items-center gap-1 text-success bg-success/10 border border-success/20 rounded px-1.5 py-0.5 font-semibold">
      ...
      <div className="flex flex-wrap gap-3 sm:gap-4 text-xs text-muted-foreground pt-2 px-1">
        <div className="flex items-center gap-1.5 font-medium">
          <span className="inline-block h-3 w-3 rounded-sm bg-success shadow-sm" />{" "}
          Active / Scheduled
        </div>
        <div className="flex items-center gap-1.5 font-medium">
          <span className="inline-block h-3 w-3 rounded-sm bg-warning shadow-sm" />{" "}
          Cancelled
        </div>
        <div className="flex items-center gap-1.5 font-medium">
          <span className="inline-block h-3 w-3 rounded-sm bg-destructive shadow-sm" />{" "}
          Refunded
        </div>
        <div className="opacity-70 italic">
          Numbers show counts for each category per day
        </div>
      </div>
    </div>
  );
